...
class GPSInfo(object):
    ...
    def __validateAlt(self, alt):
        """Validate alt"""
        if not ((isinstance(alt, int)) or (isinstance(alt, float))):
...
class GPSPhoto(object):
    ...
    def __getGPSData(self):
        ...
        def parseItude(tude):
            ...
            if len(tude) == 3:
                ...
                coordList.append(int(tude[1].strip()))
                if itude[2].find('/') > -1:
                ...
                # Deal with overflows due to rounding and float representations
                secs = coordList[2] % 60
                mins = (coordList[1] + int(coordList[2] / 60)) % 60
                degs = coordList[0] + int((coordList[1] + int(coordList[2] / 60)) / 60)
    ...
    def modGPSData(self, gpsInfo, newFileName):
        ...
        gpsDict[6] = (int(abs(gpsInfo.alt) * 1000), 1000)  # Altitude
