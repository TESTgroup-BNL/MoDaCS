...
{
	void startCam();
	void loadCal(const char folder_path[]);
	void stopCam();
...
void startCam()
{
	...
	pFrameGrabber = new ICIFrameGrabber;
...
void loadCal(const char folder_path[])
{
	...
	int nRtnError = pFrameGrabber->LoadCalibrationFile(pUSBHandler,folder_path);
...

{
    ...
    int nRtnError = pFrameGrabber->GetRadiometricImage(pUSBHandler,pRadiometricImage);
...
void stopCam()
{
	...
	pFrameGrabber->ShutdownCamera(pUSBHandler);
}